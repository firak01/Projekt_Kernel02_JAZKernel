...
public class HashMapExtendedIndexedZZZTest extends TestCase{
...
//			### Die TestObjecte	
		...
		*/
	    ...
	    public void testCheckStructure(){
			try{
				boolean btemp;
				HashMapExtendedIndexedZZZ hmIndexed = null;
				hmIndexed=new HashMapExtendedIndexedZZZ();
	    ...
	    public void testGetValueDupsIndexedByMethod(){
	    	try{
	    		...
	    		//A) HASHMAP
	    		HashMapExtendedIndexedZZZ hmIndexedA = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(hmTest, "toString");
	    		...
	    		//B) ARRAYLIST
	    		HashMapExtendedIndexedZZZ hmIndexedB = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(alTest, "toString");
	    ...
	    public void testRemoveFromDups(){
			try{
				...
				HashMapExtendedIndexedZZZ hmIndexed1 = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(hmTest1, "toString"); //Eintr�ge, wichtig ist nur, dass die Werte trotz unterschiedlicher Keys an den Stellen (s. hmTest) identisch sind.
				...
				itemp =HashMapExtendedIndexedZZZ.removeDupsFromByIndex(hmTest1, hmIndexed1, "KEEPLAST");
				...
				HashMapExtendedIndexedZZZ hmIndexed = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(hmTest1, "toString"); //Eintr�ge, wichtig ist nur, dass die Werte trotz unterschiedlicher Keys an den Stellen (s. hmTest) identisch sind.
...
//				hmIndexed  jede mal neu errechnen, da die Indexreihenfolge einem Internen Algorithmus entspricht und nicht so ist, wie notiert.
				HashMapExtendedIndexedZZZ hmIndexed2 = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(hmTest2, "toString"); //Eintr�ge, wichtig ist nur, dass die Werte trotz unterschiedlicher Keys an den Stellen (s. hmTest) identisch sind.
...
//			  TESTFALL: Behalte die ersten Eintr�ge
				itemp =HashMapExtendedIndexedZZZ.removeDupsFromByIndex(hmTest2, hmIndexed2, "KEEPFIRST");
				assertEquals("Es wurde erwartet, das 3 Objekte entfernt wurden.",3, itemp);
			    ...
			    //Jetzt soll es keine doppelten Objekte mehr geben.
				hmIndexed = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(hmTest2, "toString"); //Eintr�ge, wichtig ist nur, dass die Werte trotz unterschiedlicher Keys an den Stellen (s. hmTest) identisch sind.
				assertEquals("Es wurde erwartet, das keine doppelte Objekte existieren.",0, hmIndexed.size());
				...
				//hmIndexed  jedesmal neu errechnen, da die Indexreihenfolge einem Internen Algorithmus entspricht und nicht so ist, wie notiert.
				HashMapExtendedIndexedZZZ hmIndexed1B = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(alTest1, "toString"); //Eintr�ge, wichtig ist nur, dass die Werte trotz unterschiedlicher Keys an den Stellen (s. hmTest) identisch sind.
				assertEquals("Es wurde erwartet, das 5 doppelte Objekte existieren.",5, hmIndexed1B.size());
				iStartSize = alTest1.size();   
				
				//TESTFALL: Behalte die letzten Eintr�ge
				itemp =HashMapExtendedIndexedZZZ.removeDupsFromByIndex(alTest1, hmIndexed1B, "KEEPLAST");
				assertEquals("Es wurde erwartet, das 3 Objekte entfernt wurden.",3, itemp);
			    ...
			    //Jetzt soll es keine doppelten Objekte mehr geben.
				hmIndexed = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(alTest1, "toString"); //Eintr�ge, wichtig ist nur, dass die Werte trotz unterschiedlicher Keys an den Stellen (s. hmTest) identisch sind.
				assertEquals("Es wurde erwartet, das keine doppelte Objekte existieren.",0, hmIndexed.size());
				...
				//hmIndexed  jedesmal neu errechnen, da die Indexreihenfolge einem Internen Algorithmus entspricht und nicht so ist, wie notiert.
				HashMapExtendedIndexedZZZ hmIndexed2B = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(alTest2, "toString"); //Eintr�ge, wichtig ist nur, dass die Werte trotz unterschiedlicher Keys an den Stellen (s. hmTest) identisch sind.
				assertEquals("Es wurde erwartet, das 5 doppelte Objekte existieren.",5, hmIndexed2B.size());
				iStartSize = alTest2.size();   
				
				//TESTFALL: Behalte die ersten Eintr�ge
				itemp =HashMapExtendedIndexedZZZ.removeDupsFromByIndex(alTest2, hmIndexed2B, "KEEPFIRST");
				assertEquals("Es wurde erwartet, das 3 Objekte entfernt wurden.",3, itemp);
			    ...
			    //Jetzt soll es keine doppelten Objekte mehr geben.
				hmIndexed = HashMapExtendedIndexedZZZ.getValueDupsIndexedByMethod(alTest2, "toString"); //Eintr�ge, wichtig ist nur, dass die Werte trotz unterschiedlicher Keys an den Stellen (s. hmTest) identisch sind.
				assertEquals("Es wurde erwartet, das keine doppelte Objekte existieren.",0, hmIndexed.size());
