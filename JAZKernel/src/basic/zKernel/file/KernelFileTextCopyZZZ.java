...
public class KernelFileTextCopyZZZ   extends KernelUseObjectZZZ{
		...
		main:{
			...
			if(saFlagControl != null){
						for(int iCount = 0;iCount<=saFlagControl.length-1;iCount++){
							...
							if(btemp==false){ 								   
								   ExceptionZZZ ez = new ExceptionZZZ( sERROR_FLAG_UNAVAILABLE + stemp, iERROR_FLAG_UNAVAILABLE, this, ReflectCodeZZZ.getMethodCurrentName()); 
			...
			//Falls das SourceDirectory null ist, wird davon ausgegangen, dass saFileSource die Pfade enthält
			...
			//Falls das TargetDirectory null ist, wird davon ausgegangen, saFileTarget die Pfade enthält
	 ...
	 */
	public void setKernelObject(KernelZZZ objKernelIn) {
	...
	public boolean startit(String[] saFlagControl) throws ExceptionZZZ{
		...
		main:{
			...
			for(int i = 0; i <= saFileSource.length-1; i++){
     		  handlefile:{ //damit man zur nächsten datei gehen kann mit break!!!
				...
				sFileSourceBasis = objFileTemp.getName(); 
				   ...
				   if(sDirectoryTarget==null){
					...
					sDirectoryTargetFinal = objFileTemp.getPath();
				   		...
				   		//Fall: Target-Directory explizit übergeben
				   ...
				   if(sDirectorySource==null){
					   ...
					   if(objDirTemp.isDirectory()==false){
						...
						sDirectorySourceFinal = objDirTemp.getPath();
				   		...
				   		//Fall: Source-Directory explizit übergeben
					   ...
					   do{ //Solange es ein Source-File mit einer "Folge"-Expansion gibt, soll kopiert werden.       		        	
						   ...
						   sFileSourceExpanded = objFileSource.PathNameTotalExpandedFirstCompute();
        					...
        					if(objFileTemp.exists()==false){
								// Falls die letzte SourceDatei schon gelöscht wurde, wird der Originale-Source-Dateiname als Expansion zurückgegeben, doch der ist schon längst nicht mehr da.
								bGotoNextExpansion = false;		
        					}else if(sFileSourceExpandedLast.equals(sFileSourceExpanded)){
								//Falls die Source-Datei nicht gelöscht oder umbenannt wurde, wird so die Quelldatei immer wieder aufgegriffen, also:
